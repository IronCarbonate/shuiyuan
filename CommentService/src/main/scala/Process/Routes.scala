...
import Impl.LikeCommentMessagePlanner
import Impl.CancelLikeCommentMessagePlanner
import Impl.QueryCommentDetailsMessagePlanner
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "LikeCommentMessage" =>
        IO(
          decode[LikeCommentMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for LikeCommentMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "CancelLikeCommentMessage" =>
        IO(
          decode[CancelLikeCommentMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for CancelLikeCommentMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "QueryCommentDetailsMessage" =>
        IO(
          decode[QueryCommentDetailsMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for QueryCommentDetailsMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
